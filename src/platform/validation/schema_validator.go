...
//
// The published beckn.yaml is the validator, and nothing here restates a rule the
// document already carries: a hand-rolled second copy drifts silently, because
// both copies keep passing their own tests.
//
// Every fault comes back, each against its own path, for a caller to chain through
// details.cause (C7).
func L1(index *SpecIndex, action string, body []byte) []*apperrors.AppError {
	...
	if err := json.Unmarshal(body, &document); err != nil {
		...
		// validator that fails on the body it was handed is a 500 where a 400
		// belongs.
...
//
// C2 says `publish` and `catalog/publish` are one request; the document says the
// field equals `catalog/publish` (A12). Reconciling them here keeps that document
// the single source of truth, and doing it in a freshly decoded map rather than in
// the buffered request body keeps the bytes this service signs, hashes and audits
// identical to the ones the caller sent.
//
// An absent action is left absent: minting one would hide the omission from the
// envelope rules, the only pass that can report it (C6).
...
func faultFrom(document any, err error) *apperrors.AppError {
	...
	if reason == "" {
		// Rare, and the path still names the field, so a caller is not left with
		// nothing.
...
// schemaFault builds the one fault every L1 rejection is. The code is a literal
// rather than a parameter so the minted-codes pin in src/platform/errors can see
// it: a code reaching a family constructor through a variable is invisible to that
// walk, and the walk is what keeps a SCH_ code from being reported as a CTX_ one.
...
// say whether it is a member name or an array index — the visitor reports both as
// bare strings, and a catalog keyed "0" is a real shape. Only the walk can render
// `spatial[0]` rather than `spatial.0`, which no tool the caller owns evaluates.
...
// it selects. A segment that selects nothing yields nil and the rest render as
// member names — a full path beats one truncated where the walk lost its footing.
...
// carry it. `@type` and `beckn:id` are both real keys in this protocol and neither
// is a legal shorthand, so `$.message.@type` reads fine and evaluates to nothing.
