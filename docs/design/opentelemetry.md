...
|---|---|
| `eid` | `API` |
| `producer` | This deployment's participant id, e.g. `discovery-service`. New config. Participant-id shape — `^[a-z0-9][a-z0-9._:-]{2,252}$`, `maxLength` 253, from `docs/registry/schemas/ProviderSchema.json#/$defs/ParticipantId`. **Not** the `{2,63}` alternation on that file's line 18: that is a *provider* id, and open question 3 says a discovery service is not a Provider. The pattern is borrowed as a shape, not resolved from a record — there may be no record for us to resolve |
