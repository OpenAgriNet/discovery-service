...
| 25 | Node-Operator Metrics | **Not blocked, and not Task 24.** Task 24 is the facilitator's METRIC signal, gated on a `metric.code` registry; this is the `onix_*`-style operational set a node operator needs — the saturation **levels** of the three ceilings, Postgres dependency health, one liveness gauge, and a duration histogram *conditional on Decision 6*. Conflating the two is why this service has no metrics at all today. **Do not repeat the claim that a 429 produces no span** — it does: `Trace` is index 1 in `router.go:134-141`, above `Envelope` and `RateLimit`, both of which refuse through `httpx.WriteNack` → `logNack`, which is where 23d's `error` event comes from. The justification is that a ceiling is a level and a span is an event, so nothing reports a pool at 30 of 32 while every request succeeds. Tests pin the levels: acquire-wait under a pool sized to 1, an in-use gauge that moves with nothing refused, and an assertion that **no instrument here counts a rejection**. Per-provider freshness and the alert list are **struck** — the first is a `SELECT` and a cross-replica aggregate, the second is a rule file with no acceptance criterion. See `opentelemetry.md` **OP1, OP3, OP4**, and **OP2 only if Decision 6 keeps sampling configurable** |
