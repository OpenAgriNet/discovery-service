...
`storage/postgres/`, `middlewares/ratelimit.go` and `middlewares/envelope.go`
...
|---|---|---|
| Connection pool | `DATABASE_MAX_CONNS` (32) | Acquire-wait is invisible; exhaustion shows up as latency with no cause attached |
| Request rate | `RATE_LIMIT_RPS` / `RATE_LIMIT_BURST` (20 / 40) | A 429 is written by `RateLimit`, **above** the handler — no span, no event, no counter |
| Body size | `SERVER_MAX_REQUEST_BODY_BYTES` (10 MiB) | Same: `Envelope` refuses before anything downstream records the attempt |

That middleware-above-the-handler detail is the whole reason this is a task
rather than a dashboard query over Task 23's spans. **The failure modes an
operator most needs to see are the ones that produce no span**, so no amount of
aggregation over spans recovers them.

What it emits, all under our own scope rather than the global meter:

- **RED per action** — request count, error count, duration histogram, keyed on
  the action already on the span.
- **Saturation** — pool acquire-wait and in-use count; rate-limit rejections;
  body-ceiling refusals. The last two are counters that do not exist today.
- **Dependency health** — Postgres query duration; Ollama when semantic is on.
  `retrieval.embedding_ms` covers Ollama only while it is enabled and nothing
  covers Postgres.
- **Freshness per provider** — age of the newest publish, keyed on
  `publish.bpp_ids`. In agriculture a stale weather catalog is worse than an
  absent one: it answers confidently and wrongly.

**Tests pin the rejected requests**, because those are the ones the current
design loses: a 429 and a body-ceiling refusal each increment their counter, and
acquire-wait is observed under a pool deliberately sized to 1.
