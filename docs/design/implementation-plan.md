...
imports a driver, so the backend is swappable. HTTP is `net/http` alone — the
chi dependency named here was never added and **A20** records why; SQL is
sqlc + pgx; wiring is explicit constructors, no reflection.

**Tech Stack:** Go 1.25 · `net/http.ServeMux` (A20) · pgx/v5 + sqlc · PostgreSQL 16 + pgvector 0.8
...
- [Appendix — Rationale Recovered From Source Comments](#appendix--rationale-recovered-from-source-comments)
...
| **C6** | `Context` declares no `required` list, so L1 alone cannot reject a missing `transactionId` | Envelope rules are enforced separately by struct tags: `action`, `version`(=2.0.0), `messageId`(uuid4), `transactionId`(uuid4), `timestamp`(RFC3339) required. `senderId`, `receiverId`, `networkId` stay optional (**A24** — the two participant DIDs are unverified in this phase, and a required field this service cannot check would demand an identity and then take the caller's word for it; `networkId` is a filter with a default, C8). Runs even when L1 is off. **The same absence is what lets OAN decline `bapId`/`bppId` in `context` at no cost.** The spec's prose says the context "MUST include at minimum … `bapId` or `bppId`" (`:66-72`), and its machine-readable half says nothing at all: `Context` carries no `required`, no `additionalProperties` and no `oneOf`/`anyOf`, so an envelope with `senderId`/`receiverId` and neither legacy id validates cleanly. `senderId` and `receiverId` are themselves declared properties of that same schema — OAN is *selecting from* the spec's own property list, not deviating from it. This is one of the few places the spec's prose and its schema disagree with each other rather than with us, and the rule "the schema wins" resolves it in OAN's favour. Unlike `Catalog`, which closes with `additionalProperties: false` and so pins `bppId` as the only accepted spelling there |
...
| **T2** | §6, §7 Observability | OpenTelemetry traces, W3C Trace Context in and out, OTLP exporter (default `none` so a collector-less deploy still boots). **No *request-rate* metric is emitted from this process — see A23**: RED figures per route are computed downstream from the spans, because a stateless replica's in-memory counter is a partial nobody can reassemble. This row read "Metrics are not emitted from this process" until 2026-09-10, which is now too strong in both directions — Task 25 shipped two pool-saturation instruments, which are *levels* and not partials, and Task 24's five network codes are derived in `otel/collector.yaml` rather than downstream of the node. Dashboards are out of scope | 20, 23, 24, 25 |
...
| §7 | **Long-term telemetry archive** — retaining the span stream in a form someone can download and load into a system that is not ClickStack. An `awss3` exporter writing OTLP-JSON (gzipped) to S3 or MinIO, in whichever collector the deployment runs; `awss3` is contrib, not core, so the image matters. No code here: this service exports OTLP to one endpoint and every destination past it is configuration. Note there is no collector of ours in the path at all — Task 23's decision is **direct export** to ClickStack and the facilitator, and ClickStack bundles its own collector, so the archive goes into that config or into one placed in front. Two things a reader will get wrong: exporters in a pipeline **fan out, they do not chain**, so an archive "before ClickStack" is a sibling pipeline off the same receiver rather than a stage in front of it; and the ClickStack stream is the **un-deny-listed** one, because 23f enforces the deny-list on the *facilitator* exporter — so an archive tapped off it carries the attributes the deny-list exists to withhold. That makes "someone loads it into their system" a confidentiality decision before it is a config one, and the decision is not the platform team's to make alone | Platform, on a confidentiality call that is **ours** — see the never-emitted list under 23f |
...
|---|---|---|
| `CONTRIBUTING.md`, `SECURITY.md`, `CHANGELOG.md`, `LICENSE` | Contribution policy and disclosure timelines are decisions the project has not made. A placeholder `SECURITY.md` publishes a promise. `LICENSE` joined them rather than shipping: the licence has not been chosen, and it is the one file where a placeholder cannot be quietly corrected later — a grant, once published, is relied upon. An absent licence grants nothing, which is the honest default; a wrong one grants something the project did not agree to | `README.md`, with a Licence section that says the decision is open |
...
| `otelpgx` for database spans | **A spike, not a commitment, and not a gate on anything.** The 2026-09-09 build-vs-reuse audit found `github.com/exaring/otelpgx` v0.11.1 the one library of six worth real investigation: it is a `pgx` tracer, which is exactly the seam `storage/postgres/pool.go` already has, and it would supply per-query spans this service does not emit. Two things must be answered before any of that matters, and the first is fatal if it goes the wrong way: **does it obtain its own tracer?** If it does, A23's `scope.version` collision applies to it exactly as it does to `otelhttp` — the span carries the library's version where the network spec wants `1.0` — and the answer is no. It is **not** a reason to revisit Task 25's two pool counters, which are a levels signal `otelpgx` does not produce | Nothing. The pool instruments Task 25 shipped by hand, and no DB-query spans |
| `fact.Definition.Visibility` | **Deleted on 2026-09-09, and 23f is the task that reintroduces it.** It was built to govern 23f's facilitator deny-list, 23f is blocked on O1/O2/O4, and in the meantime the column had **zero runtime consumers** — `telemetry/redact.go` is not a file. Two things settled it: `fact.go` documented behaviour that did not exist ("the projection treats Unspecified as LocalOnly at runtime"), and all 54 rows carried `Public` with none ever carrying `LocalOnly`, so the column had never distinguished one row from another. Every new row still had to choose a value for it. Re-adding it is the same mechanical work in reverse — one type, one struct field, N declarations, one golden regeneration — and 23f is the commit that will know what the deny-list actually needs, which today nobody does. The design it encoded is preserved here so 23f restores rather than re-derives it: `type Visibility uint8`, with `const (VisibilityUnspecified Visibility = iota; Public; LocalOnly)`. The zero was `Unspecified` and the projection was to treat `Unspecified` as `LocalOnly` at runtime, because a forgotten field must not mean "exported" | Nothing, and **nothing is lost:** the deny-list was never enforceable from a per-row column anyway. The risk is in attribute *values* — a caller-supplied `beckn.schemaContext` URI, a `status.message`, a `zap.Error(err)` — not in key names, which is why Task 26 asserts over the **serialised bytes** of the facilitator projection. That test was always the enforcer; the column was insurance beside it |
| `Replication.Targets` has no consumer | **Found by the 2026-09-09 config audit.** `CatalogReplicator` (A7) is a real seam — Task 20 constructs it and `publish.Service` calls `Replicate` after every commit — but `container.go` wires `NoopReplicator` unconditionally and reads nothing off `cfg.Replication`. The field cannot select a second store because no task has built one yet, so naming a target here is accepted by the config loader and does nothing: no replication happens and the boot does not refuse it either, despite an earlier version of `config.go`'s comment claiming it would. The comment is corrected (2026-09-09); the field itself stays, because the seam it configures already ships (A7) and removing it now would mean re-adding it the moment a second store lands | `NoopReplicator`, regardless of `REPLICATION_TARGETS`. The task that builds the second store is also the task that makes this field do something and, at that point, decides whether a named target with no matching implementation should fail the boot |
...
|---|---|---|---|---|
| D1 | Router | ~~chi v5 (MIT)~~ **`net/http.ServeMux` — see A20 and ADR-0016** | Fiber, Gin, Gorilla | chi is `http.Handler` all the way down, so `r.Context()` propagates natively and `httptest`/`otelhttp`/`testcontainers` need no adapters. Fiber's fasthttp forces a `UserContext()` dance |
...
-- resource_geometries. With GIN's default fastupdate = on, inserts land in a pending
...
-- resources: a pending list is scanned by every search and flushed by none
...
repo.UpsertCatalog(ctx, patch, updateMode, derive):
    ...
    if updateMode == FULL:
        ...
        # in that order — see the offers schema. Still needed alongside the check
...
CATALOG-WIDE when empty, in the hydration query and in the offers schema, and an
    ...
    SELECT 1 FROM resource_geometries g
     WHERE g.catalog_id = r.catalog_id
       ...
       AND @match_negate::boolean <> (
              ...
              -- six is never compared — see the SQL functions schema.
...
`CHECK (cells_cover IS NULL OR cardinality(cells_cover) > 0)` on `resource_geometries`.
...
Twenty-six tasks, dependency-ordered — 23 as originally planned, 24 added by
A23 and 25 and 26 by A25. Each is one reviewable unit ending in a
...
`.golangci.yml`, `.github/workflows/ci.yml`, `README.md`,
...
- **The bucket is keyed on the remote address, not the subscriber id.** This is
  ...
  signature verification is parked: `context.senderId` on an unverified request
  is a string the caller chose — a DID, but nothing here resolves it (A24).
  Keying on it would let any caller shed its own limit by rotating the field,
  and — the worse half — exhaust a *named third party's* bucket by claiming
  their id, which turns the protection into the attack. Subscriber-id keying is
  in **Deferred**, tied to the task that verifies the signature, because that
  is the point at which the id stops being a claim and becomes an identity. A deployment behind a proxy must hand the
...
- `Trace` is a **no-op pass-through here**, but not side-effect-free: before
  ...
  link in the chain no order test could place. **This marker is transient:**
  Task 23c gives `Trace` a side effect of its own — the server span — and drops
  it, moving the header constant to `recover.go`, which is then its only writer.
  **`Recover` appends `recover`
...
it catches (no other behaviour to test yet — Task 23 pins the real span, and
removes `Trace`'s entry when it does, leaving `Recover` the only writer).
...
on `resource_geometries`. It no longer asserts the
...
`offers.resource_ids` invariant on `offers`, for the same reason and in the
same place.
...
**Produces:**
`postgres.NewCatalogRepository(pool, resolutionCells int) → domain.CatalogRepository`

The resolution is a constructor parameter and not read from config inside the
adapter: the H3 cover is computed on the write path, so the repository needs it,
and reaching for `config.Indexing` in there would put a config dependency behind
the port. This signature was one parameter short until 2026-09-09; the code has
always had two.
...
### Task 21b — Verbatim Document Storage & Round-Trip Fidelity

**Files:** `migrations/000001_initial_schema.up.sql` (and its `.down.sql`) — the
`catalogs`, `resources` and `offers` sections,
`src/storage/postgres/queries/{publish,discover}.sql`, `src/storage/postgres/gen/*`,
`src/domain/{catalog,mergepatch}.go`, `src/publish/{mapper,service,text,geometry}.go`,
`src/storage/postgres/{mapping,hydrator}.go`, `src/storage/memory/repository.go`,
`src/discover/service.go`, `src/storage/conformance/{publish,discover}.go`,
`tests/acceptance/roundtrip_test.go`

A17, made real. **What a publisher sends comes back byte-equivalent, and every
column beside `document` is an index over it.**

**The rule, stated once:** each published object is stored verbatim in exactly
one row's `document`. `catalogs.document` is the Catalog with `resources` and
...
`Resource.Document`; `Offer.Document` is already right and does not move. The
two dropped fields come back as **pure accessors** — `Resource.Descriptor()`,
`Resource.ResourceAttributes()`, `Catalog.Provider()` — each a shallow read of
one member. They exist so `deriveSearchText`, `schemaOf`, `descriptorName` and
the geometry walk keep their exact present semantics: the walk must still
descend `descriptor` and `resourceAttributes` under those two path segments and
must NOT start emitting a geometry found beside them, and the search text must
still be descriptor + attributes and not gain the resource `id`.

`CatalogPatch.Provider` and `ResourcePatch.{Descriptor,Attributes}` collapse the
same way, into one `Document` each. **This makes `mergeResources` one
`patchDocument` call instead of two, and the result is identical** — RFC 7396
recurses, so a patch of `{"descriptor":{"name":"x"}}` against the whole document
...
the document from its parsed fields would reintroduce exactly the lossiness this
task removes, one field at a time, and no test that compares parsed values would
catch it.

**Render.** `render` in `src/discover/service.go` stops assembling a catalog
from columns and instead emits `catalogs.document` with `resources` and
`offers` spliced back in from their own rows. This is the half that delivers
round-trip fidelity; without it the task stores data nobody reads.
...
- The nil-receiver tolerance survives: a middleware mounted without either
  allocator above it still records nothing rather than panicking.
- **Adopt or allocate.** `Trace` and `RequestLogger` both look for a record in
  the context and allocate one only if they find none, so the outer of the two
  owns it and both can read it. This is what makes the status — the one span
  attribute readable from neither the request nor a header — reachable from
  `Trace`, which sits above the `responseRecorder` that captures it. Moving
  allocation up to `Trace` outright fails the acceptance below:
  `request_logger_test.go`'s `TestTheCompletionLineCarriesTheCorrelatorsEnvelopeParsed`
  and `TestTheCompletionLineSurvivesABodyThatNeverParsed` mount
  `RequestLogger(Envelope(…))` with nothing above, and would lose their
  correlators.
...
`src/platform/validation/http_fetcher.go`,
`src/indexing/embeddings/ollama.go`

**Produces:** the span, its mandatory attributes, and the scope

- **Hand-rolled, not `otelhttp`** (A23): the instrumentation scope is immutable
  once the span exists, so a span `otelhttp` started carries that package's
  identity permanently — and `scope.version` is where the spec wants the
  *network telemetry specification's* version, in the field OTel defines as the
  instrumentation library's, so `otelhttp` would report `v0.69.0` where `1.0`
  belongs. Not because the spec makes scope mandatory: the block is Optional
  and only its two fields are Required within it (corrected by the 2026-09-09
  audit). This replaces Task 8's no-op `Trace` body and drops the
...
- `sender.id` reads `context.senderId` — **not** `bapId`/`bppId`, which A24 took
  off `beckn.Context` entirely. It has no reliable source in this phase —
  identity was parked with Task 6 — so it is **omitted rather than blanked**,
  with `sender.unidentified` set. A field that is empty on every request is one
  nothing can be filtered by, and an invented id is worse than an absent one.
- **`sender.unverified` is set whenever `sender.id` is.** Nothing here resolves
  the DID, so the id is a claim the caller made, and it is the facilitator's
  cross-participant join key. Task 8 refused to key the rate limiter on the same
  string for the same reason; telemetry must not quietly do what the rate limiter
  declined to.
- `http.status.code` is **not readable from where `Trace` sits** — it is not a
  header, and the `responseRecorder` that captures it is mounted below. It comes
  off 23b's record. See `opentelemetry.md` §*How the span learns the status*;
  getting this wrong yields a span that reports 200 for every request including
  the rejected ones, which no assertion on a success path notices.
- W3C Trace Context propagated **in and out**: a correlation id that stops at the
  ...
  (TRD §6). **In** is the inbound `traceparent`. **Out** is the two outbound
  clients this repository has — `http_fetcher.go` and `ollama.go` — neither of
  which runs under Phase 1 defaults, so the pin is a unit test on the injection
  rather than an end-to-end one. This service makes no outbound *protocol* hop.
...
`scope.name` and `scope.version` are ours and not a dependency's;
`http.status.code` on a **rejected** request reads the status actually written
and not 200; the recovered panic's 500 is inside the exported span; A11's
behavioural pin still holds.
...
- **Controllers call `record()` and nothing else.** They do not start spans, add
  ...
  events using the timestamps the record already holds. **Read literally that
  also bans the attribute registry a controller must name a key from, so the
  rule is narrowed to its purpose:** controllers never import the OpenTelemetry
  SDK and never import `src/platform/telemetry` itself; they import
  `src/platform/telemetry/fact`, whose entire dependency set is `context`, `iter`
  and `time`. `tests/architecture/boundary_test.go` is where that is pinned —
  `otelOnly` carries the ban and `factIsNotTheSDK` the one exception.
...
- **Participant identity travels as the provider-node id, never as `provider`.**
  ...
  (`src/beckn/types.go`'s `Catalog`) survives A24 precisely because it describes the
  ...
  **Both attributes read that field and neither repeats its name.** OAN does not
  ...
  (`tests/testdata/beckn-v2.0.0.yaml`, its note on legacy terminology) is a
  reason that does not apply to us,
  ...
  (`src/beckn/types.go`'s `Context` — the latter are ignored rather than modelled), and
...
`X-Beckn-Error-Type` header byte for byte; `retrieval.embedding_ms` is **absent
rather than zero** under `EMBEDDING_PROVIDER=noop`, since a zero reads as a fast
embedding rather than no embedding; `result.provider_ids` is **distinct and
bounded at 16**, so a discover answering with two hundred catalogs from one
provider emits one id rather than two hundred — an identity attribute that scales
with page size is a second, worse copy of `result.catalog_count`.
...
### Task 24 — Metrics Exporter — **five codes shipped, the rest still BLOCKED**
...
**Five codes shipped on 2026-09-10**, and the heading above used to read
"BLOCKED, and probably not this repo" without qualification. Both halves of that
turned out to be half-true, which is why the row is split rather than flipped:

| | |
|---|---|
| Shipped | `discover_api_total_count`, `publish_api_total_count`, `discover_api_failure_percent`, `publish_api_failure_percent`, `discover_api_empty_result_percent` — the five OAN owns and runs the collector for. Registry in `opentelemetry.md` §The five codes |
| Still blocked | The other seven of the twelve proposed codes, and the seven questions in `opentelemetry.md` §Proposed codes. O1 and O2 still gate them |

**And it was not this repo, in the sense that mattered.** No Go shipped: no
instrument, no `fact.Instrument` row, no exporter component. The five are
derived from Task 23's spans entirely inside `otel/collector.yaml` — a `sum`
connector over 0/1 span flags, `metricsgeneration` for the three ratios, an
allow-list, and OTTL to shape the result. The prediction that the aggregation
belongs outside a stateless replica held; what it got wrong is that the node's
own collector is *already* outside it, so no new tier had to exist first.

`tests/otelpipeline` runs that config against a real collector and reads back
what it exports. It is the only thing pinning the derivation — the YAML has no
unit test and cannot have one.

What still does not exist is the general form of the thing below, for any code
whose window is longer than the collector's or whose input is not a span. A
...
**The remaining seven are blocked on a metrics registry**, which does not exist
for OAN. `metric.code` is defined there; invented codes will not match the ones a
facilitator later publishes, and a stream of unrecognised codes is worse than
none. The five above are exempt from that argument for one reason only: OAN owns
the registry and runs the collector, so those five *are* its first entries rather
than a guess at someone else's.

`opentelemetry.md` §Proposed codes — the outward proposal is the proposal for
that registry — twelve
codes derived from Task 23's spans, and seven questions whose first two block
the rest. It does not unblock them: it is what to send to whoever owns the
...
**Produces:** `src/platform/telemetry/metrics.go`, plus instrumentation in
`storage/postgres/` — `middlewares/ratelimit.go` and `middlewares/envelope.go` leave the list with the rejection counters

A23 removed in-process metrics because a stateless replica cannot compute a
windowed network aggregate. That argument is sound and it covers exactly one of
the two audiences. The facilitator wants `search_api_total_count` across the
network; the person on call wants to know whether *this replica* is about to
fall over, and that number is per-replica by definition — there is nothing to
reassemble. Treating both as one blocked task is why this service currently
emits no metrics at all.

Three configured ceilings exist and not one is observable:

| Ceiling | Config | Today |
|---|---|---|
| Connection pool | `DATABASE_MAX_CONNS` (32) | Acquire-wait is invisible; exhaustion shows up as latency with no cause attached. **The one ceiling nothing observes at all** |
| Request rate | `RATE_LIMIT_RPS` / `RATE_LIMIT_BURST` (20 / 40) | The *refusal* is already visible — `RateLimit` sits inside `Trace`, so a 429 is a span with a status and an `error` event. The *headroom* is not: nothing reports a limiter at 18 of 20 |
...
**What survives is a difference in kind, not in coverage.** A span is emitted
because something happened; a ceiling is a level that exists between the things
that happen. No aggregation over spans reports a pool that sat at 30 of 32 for ten
minutes while every request succeeded, because that interval contains no event to
aggregate. Sampling a gauge on a clock is the only instrument that reports it, and
that is what this task builds.

**Second, and only since Decision 6.** Sampling is now a deployment's knob rather
than a rebuild's. Every rate read off span counts is therefore a rate times an
unknown fraction, and the fraction is smallest exactly when traffic is highest. A
duration histogram is not sampled and merges across replicas; a percentile
computed over stored spans is both.

**Third, negatively, and it is what shrinks the list.** `opentelemetry.md`'s *No
duration attribute* rule refuses a second copy of a fact the span already carries,
because a second copy is free to disagree. That rule does not stop at attributes.
A rejection counter beside a spanned rejection is the same object one signal up,
and it drifts the first time a refusal path is added without an `Inc()`. So no
counter here restates a span fact.

What it emits, all under our own scope rather than the global meter:

- **Saturation levels** — pool acquire-wait and in-use count. These are the
  content of the task: a distance to a ceiling, sampled on a clock, reported
  between the events spans already cover.
- **Dependency health** — Postgres query duration. Covered by no span and no event
  today; the pool half is a poll of `pgxpool.Stat()` and costs almost nothing.
  Ollama is *not* here: `retrieval.embedding_ms` already covers it, and only while
  semantic is on, which is never in Phase 1 (A5).
- **Request duration per action**, as a histogram — **conditional on Decision 6.**
  It earns a metric only because a sampled span stream cannot be trusted for
  percentiles and a merged histogram can. If Decision 6 comes back "never sample"
  and ClickHouse retention is long enough, **strike this bullet**; it is the one
  item here whose justification lives in another decision.
- **One liveness gauge** — a node that stopped receiving looks exactly like a
  network that went quiet, and only a value emitted on the meter's own interval
  tells them apart. One instrument, and it must be held to beating `/readyz`,
  which already answers "up" to the orchestrator.

**Three things that were on this list and are struck**, because saying why is
worth more than a shorter list. *Rejection counters* — the 429 and the body
refusal are already spanned; see above. *Freshness per provider* — it is
`SELECT max(published_at) … GROUP BY bpp_id`, the answer P9 already gives for the
same shape of question, and as a metric it fails A23's test rather than escaping
it: each replica sees only the publishes it handled, so the deployment-wide answer
is a windowed aggregate over N partials, which is Task 24's definition. It would
also make a provider id an unbounded metric label. *The alert list* (OP10) — a
rule file is not Go, `make test` can assert nothing about it, and it depends on
the SLO Decision 7 refuses to invent. It follows this task; it is not in it.

**Tests pin the levels, not the refusals**: acquire-wait is observed under a pool
deliberately sized to 1, the in-use gauge moves with no request refused, and a
test asserts that no instrument here counts a 429 or a body refusal.

Needs the served-request SLO (`opentelemetry.md` Decision 7) before the alert
list of OP10 means anything — but not before the instruments exist. Cannot
precede Task 23, which establishes the scope and the Resource these share.

---

### Task 26 — Deny-List Conformance

**Produces:** `src/platform/telemetry/redact_test.go`

The never-emitted list — `textSearch`, `filters.expression`, raw coordinates,
user agent, anything resembling a farmer's identity — is currently a rule that
23a through 23f comply with and nothing checks. A privacy constraint held only
by everyone having remembered it is not a constraint, and this repo's standing
rule is that such a pin gets encoded.

One fixture-driven test over the **facilitator exporter's output**: build a span
carrying every denied field, run it through the exporter, assert none survives.
Asserting over the exported payload rather than over the code that builds it is
the point — it catches an attribute added later by a path this test never heard
of, which is the only failure mode that matters.

Follows 23f, which builds the exporter it asserts against. Fixture-driven so
that adding a denied field is a fixture line rather than a new test.

---

## Open Items

**Four, all network-level, all raised by Task 23.** Every other question this
plan raised has an owner decision behind it.

The line above used to read "None", with a note that it had a short shelf life
because implementation would raise more. It did. These four are not ours to
...
| **O4** | **Which trace-id format and which event-timestamp field does the facilitator validate?** The spec's examples carry dashed UUIDs (`d4ae9294-ab00-11ee-…`) and an ISO `time` on events. Neither is valid OTLP, which requires 16- and 8-byte hex ids and `timeUnixNano` — and OTLP is what every OTel SDK emits and what the spec's own design principle 2 says to adopt | 23f | **This one is not ours to fix and not ours to work around.** If the facilitator validates against the examples it will reject conformant spans from *every* participant, not just us, and the fix is a correction to the spec rather than a rewrite of our exporter. Guessing the other way — hand-rolling UUID-shaped ids to match the examples — means emitting something no OTel tool, ClickStack included, will parse. `opentelemetry.md` divergences 5 and 6 |

The A23 restructure assumes all four stay open. 23a–23e are specified so they do
not depend on any of them: the `domain` and `producer` values are config, so a
deployment can be reconfigured without a rebuild, nothing is exported to a
facilitator until 23f, and the id and timestamp formats are the SDK's own until
...
## Appendix — Rationale Recovered From Source Comments

The godoc sweep under #10 replaced long explanatory comments in `src/` with
one-line godoc. Most of what it cut was a second copy of something already in
this document. Thirteen facts were not: they existed **only** in the comment
that was deleted, and they are recorded here so that a reader who wonders why a
piece of the code is shaped the way it is finds the answer in the binding
document rather than in `git log -S`.

Each entry names the symbol it came from. Two further facts from that sweep are
absent because they were **corrections to this document** and have been applied
in place — the `NewCatalogRepository` arity in Task 15, and Task 8's note that
`Trace`'s `X-Beckn-Chain` marker is transient.

Nothing here introduces a new constraint. Where an entry and a task section
disagree, the task section wins.

### A1 · `ResourceKey` uses NUL as its separator, and nothing else is safe
...
### A2 · `TouchedSet` is a set beside a slice, deliberately, and the cost is measured
...
### A3 · A14's pointer has a specific silent failure mode

`src/domain/catalog_repository.go`

A14 says `DeriveFunc` takes `*Catalog`. What the comment recorded and the
amendment does not is *how a value parameter fails*: everything `derive`
computes it delivers by writing onto the merged catalog, and against a value
parameter that works for `merged.Resources[k].Field` — which reaches through the
shared backing array — and silently does not work for `merged.Geometries`, which
is a field assignment on a copy.
...
### A4 · `ErrRetrievalDepth` is the guard behind the guard

`src/domain/search_repository.go`

This document makes the discover mapper the owner of this refusal, and the
mapper checks the same bound against the same `config.Search`, so a request over
HTTP is turned away before a query runs. The sentinel exists for the paths that
do not come through the mapper.

Both sentinels sit on the port rather than in the adapter that raises them for
an import-graph reason: `tests/architecture` forbids `src/discover` from
importing `src/storage/postgres`, so an error declared there is one the request
path cannot match — and an error it cannot match is an error it reports as a
500.

### A5 · `PruneOfferReferences` is the only defence that catches a resource that never existed
...
### A6 · `Capability.Ranked()` decides two things neither backend would remember
...
### A7 · `pruneOrphanedOffers` never fires today, and the statement order is a bet on tomorrow

`src/storage/postgres/catalog_repository.go`

Task 15's "the delete runs before the prune" is **unobservable through the
ports** as the code stands: the domain-side `PruneOfferReferences` has already
fixed every offer the transaction can see, so the SQL prune finds nothing to do
regardless of when it runs. The order is kept because it is the order that stays
correct if the domain-side prune is ever removed — not because a test can tell
the two apart.

### A8 · A geometry with no bounding box is undiscoverable, not degraded
...
is the **entire** spatial predicate — see the spatial predicate under
[Discover](#discover--how-it-works). A row carrying no box therefore matches no
spatial query at all, and nothing in the response would say so.

### A9 · `callersFilter` must not key on `PgError.Routine`
...
between minor versions would turn a 400 back into a 500 silently. The SQLSTATE
is the documented contract.

### A10 · `application_name` is a literal and must not become `config.App.Subscriber`
...
the whole reason Task 25 ships no pool-utilisation gauge (`opentelemetry.md`,
the pool instruments section).

### A11 · The limiter's eviction is amortised, and `horizon` is derived rather than configured

`src/platform/middlewares/ratelimit.go`

Task 8 says only "evicts idle buckets so the map is not a leak". Two decisions
behind that are not written there.
...
### A12 · Per-resource geometry was the reference implementation's shape

`src/storage/conformance/publish.go`

[Publish](#publish--how-it-works) carries the measurement — three provider
...
### A13 · Declining the legacy participant fields costs nothing, and that is a schema fact

`src/beckn/types.go`, `Context.SenderID`

A24 gives every reason the four legacy participant fields went, but not the one
that makes the removal safe rather than merely preferred. The spec's PROSE says
the context "MUST include at minimum … `bapId` or `bppId`" — and `Context`
carries no `required`, no `additionalProperties` and no `oneOf`/`anyOf`, so the
demand is unenforceable and an envelope carrying only `senderId`/`receiverId`
validates clean. `senderId` and `receiverId` are declared properties of that
same schema, which makes A24 a SELECTION from the spec's property list rather
than a deviation from it. `Catalog` is the opposite case and is why
`Catalog.BppID` stays: it closes with `additionalProperties: false`, so `bppId`
is the only spelling validation there will accept.
