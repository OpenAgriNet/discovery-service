...
- **Adopt or allocate.** `Trace` and `RequestLogger` both look for a record in
  ...
  `request_logger_test.go:181` and `:207` mount `RequestLogger(Envelope(…))`
  with nothing above, and would lose their correlators.
