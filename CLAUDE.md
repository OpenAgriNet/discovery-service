...
| `docs/registry/` | `registry.md` is the main document — how it fits, the three JSON Schemas, examples, and the create/search/update/delete APIs; `schemas/` the machine-readable draft-07 files; `examples.md` the thirteen records to seed; `usecases.md` all five v1 use cases end to end. `archive/` is the BV Beckn adapter's own design set — a **different system**, copied verbatim for interop context and kept diffable against its source. Binding on nothing here; `docs/design/` still wins |
