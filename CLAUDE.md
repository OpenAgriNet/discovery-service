...
| `src/platform/` | Config, logging, errors, middleware, validation, plus `httpx` (envelope + response writer) and `jsonpath`. `telemetry/` is built — 17 Go files, and its layout is meant to be read off `ls`, with `doc.go` carrying the file map in prose: `provider.go` boots the SDK and owns both OTLP exporters and the propagator, `identity.go` is who this process says it is (the producer/domain/network triple, the Resource, the build stamp), `span.go`/`metrics.go` are one projection per signal, and `fact/` is the attribute registry (`definition.go` what a fact is **and** the rules a row must satisfy, `registry.go` the table, `record.go` what one request observed, `instrument.go` the metric instruments). One test file per source file; `provider_internal_test.go` is the exception and cannot merge into `provider_test.go` — it is `package telemetry` where the other is `package telemetry_test`. The log projection is `src/platform/logger/fields.go` and cannot move here — `tests/architecture/boundary_test.go` is what forbids it. **Three** directories are still empty placeholders holding a `.gitkeep` and no Go: `constants/`, and `crypto/signature/` and `registry/` — Task 6 is parked, so nothing declares `registry.Keyring` and `validateAuth` refuses the boot rather than let `AUTH_ENABLE_SIGNATURE_VERIFICATION` claim otherwise. This row said "four" and named `telemetry/` among them until 2026-09-09; Task 23 has since landed 23a–23e |
...
| `docs/design/` | The plan (`discover-and-publish.md`), the telemetry design (`opentelemetry.md`) and its companion `telemetry-seam.md` — the *what* and the *where the code lives*; the seam is the attribute registry that makes an attribute change one edit across span, log and metric, and it is subordinate to both `opentelemetry.md` and the plan; `telemetry-examples.md` is the worked OTLP payload for one transaction across both this service and beckn-onix, illustrative and binding on nothing, kept because it is where the cross-repo attribute divergences are recorded. Plus the driver prompts (`implementation-prompts.md`), and `registry/` — the registry schema design. `schema-revision-proposal.md` is RESOLVED and kept only for the MEASUREMENTS behind A18 and A19, which do not fit in an amendment row — do not delete it as stale. `telemetry-build-vs-reuse-audit.md` is an audit, binding on nothing and deliberately not folded into the design: it measures where the telemetry code actually is. Both its findings have since been **applied on 2026-09-09**, so read it as a record rather than a backlog. Finding 2 — `fact.Visibility` had no runtime consumer while 23f stayed blocked — was applied by **deleting the column**; all 54 rows had carried `Public` and none `LocalOnly`, so it had never distinguished anything. 23f reintroduces it, the Deferred table in `discover-and-publish.md` is the entry that says so, and `telemetry-seam.md` §3 keeps the type declaration verbatim for that purpose — do not re-add it to `Definition` ahead of 23f, and do not read its absence as a decision that the deny-list is unnecessary: Task 26's byte-level conformance test is and always was the enforcer. Finding 1 — the `scope` misquote — was applied in place: every location now says the `scope` block is Optional and the reason `otelhttp` is rejected is `scope.version`'s repurposed meaning, not a missing mandatory field. Do not "restore" the requires-on-every-batch claim; `opentelemetry.md:442-443` is the accurate sentence — read it before proposing that any of this be rewritten around a library. `metric-code-registry-proposal.md` is addressed OUTWARD and binding on nothing here: it is what to send to whoever owns the OAN metrics registry, and Task 24 stays blocked until they answer its seven questions — do not implement its twelve codes as though ratified. Everything binding lives here |
