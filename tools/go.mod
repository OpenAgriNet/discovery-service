// Module tools pins the build toolchain. It is deliberately separate from the
// service module so that the service's go.mod states only what ships in the
// binary: a licence audit (Global Constraints) and a supply-chain scan (T4)
// cannot tell a linter's transitive dependency from a runtime one once the two
// graphs are merged.
